...
export const HeroVideo = React.forwardRef<
  HTMLVideoElement,
  HeroVideoProps
>(({ style, className, transition, scaleRange = [0.7, 1], ...props }, ref) => {
  const { scrollYProgress } = useContainerScrollContext()
  const scale = useTransform(scrollYProgress, [0, 0.8], scaleRange)
...
>(
  (
    ...
    const roundedness = useTransform(scrollYProgress, [0, 0.8], roundednessRange)
